...
function ConfigBaseInfoController($rootScope, $scope, $window, $location, $translate, toastr, EventManager, UserService, AppService,
    ...
    function handlePermission() {
        ...
        //app admin or super admin can export the app
        var isAppAdmin = false;
        var isSuperAdmin = false;
        var refreshExportPermission = function () {
            $scope.hasExportAppPermission = isAppAdmin || isSuperAdmin;
        };
        PermissionService.has_assign_user_permission(appId).then(function (result) {
            $scope.hasAssignUserPermission = result.hasPermission;
            isAppAdmin = result.hasPermission;
            refreshExportPermission();
        }, function (result) {

        });
        PermissionService.has_root_permission().then(function (result) {
            isSuperAdmin = result.hasPermission;
            refreshExportPermission();
        }, function (result) {
    ...
    $scope.openAppExportDialog = function () {
        var ensureDefaultChecked = function () {
            var currentEnv = $rootScope.pageContext.env;
            var checkedAny = false;
            //default: only check the currently selected env
            $scope.appExportEnvs.forEach(function (env) {
                var matched = !!currentEnv && env.name === currentEnv;
                env.checked = matched;
                checkedAny = checkedAny || matched;
            });
            //fallback: make sure at least one env is checked so that export always goes through
            if (!checkedAny && $scope.appExportEnvs.length > 0) {
                $scope.appExportEnvs[0].checked = true;
            }
            $scope.appExportIncludeLinked = true;
        };
        if (!$scope.appExportEnvs) {
            EnvService.find_all_envs().then(function (result) {
                $scope.appExportEnvs = result.map(function (env) {
                    return {name: env, checked: false};
                });
                ensureDefaultChecked();
                $("#appExportDialog").modal('show');
            }, function (result) {
    ...
    $scope.toggleAppExportEnv = function (env) {
    ...
    $scope.exportApp = function () {
        ...
        $("#appExportDialog").modal('hide');

        var url = AppUtil.prefixPath() + '/apps/' + $rootScope.pageContext.appId
            + '/export?envs=' + selectedEnvs.join(",")
            + '&includeLinked=' + ($scope.appExportIncludeLinked ? 'true' : 'false');
        //navigate to the export endpoint to trigger the file download

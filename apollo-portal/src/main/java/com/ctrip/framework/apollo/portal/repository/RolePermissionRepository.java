...
public interface RolePermissionRepository extends PagingAndSortingRepository<RolePermission, Long> {
  ...
  @Modifying
  @Query(value = "UPDATE \"RolePermission\" SET \"IsDeleted\" = 1, \"DeletedAt\" = EXTRACT(EPOCH FROM NOW())*1000, \"DataChange_LastModifiedBy\" = ?2 WHERE \"PermissionId\" in ?1 and \"IsDeleted\" = 0", nativeQuery = true)

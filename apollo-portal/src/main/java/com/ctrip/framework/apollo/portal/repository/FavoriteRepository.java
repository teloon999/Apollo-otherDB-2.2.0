...
public interface FavoriteRepository extends PagingAndSortingRepository<Favorite, Long> {
  ...
  @Modifying
  @Query(value = "UPDATE \"Favorite\" SET \"IsDeleted\" = 1, \"DeletedAt\" = EXTRACT(EPOCH FROM NOW())*1000, \"DataChange_LastModifiedBy\" = ?2 WHERE \"AppId\"=?1 and \"IsDeleted\" = 0", nativeQuery = true)

...
public interface ConsumerRoleRepository extends PagingAndSortingRepository<ConsumerRole, Long> {
  ...
  @Modifying
  @Query(value = "UPDATE \"ConsumerRole\" SET \"IsDeleted\" = 1, \"DeletedAt\" = EXTRACT(EPOCH FROM NOW())*1000, \"DataChange_LastModifiedBy\" = ?2 WHERE \"RoleId\" in ?1 and \"IsDeleted\" = 0", nativeQuery = true)
  int batchDeleteByRoleIds(List<Long> roleIds, String operator);
}

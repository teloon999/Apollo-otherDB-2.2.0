...
public class ConfigsImportService {
  ...
  // lenient gson that accepts the fixed export date format as well as the legacy
  // locale-dependent default gson date formats (e.g. "May 11, 2026, 4:18:42 PM")
  private Gson gson = new GsonBuilder()
      .registerTypeAdapter(Date.class, new LenientDateTypeAdapter())
      .create();
  ...
  /**
   * Date type adapter that tolerates the fixed export date format and the legacy
   * locale-dependent default gson date formats, so that files exported by JVMs with
   * any locale/JDK version can be imported successfully.
   */
  static class LenientDateTypeAdapter extends TypeAdapter<Date> {
    ...
    private static final Locale[] LOCALES = {Locale.US, Locale.ENGLISH, Locale.CHINESE, Locale.SIMPLIFIED_CHINESE,
        Locale.getDefault()};
      ...
      for (String pattern : PATTERNS) {
        for (Locale locale : LOCALES) {
          try {
            ...
            // try next pattern/locale
          }
        }
      }
      throw new JsonParseException("Failed parsing '" + dateStr + "' as Date");
    }
  }
}

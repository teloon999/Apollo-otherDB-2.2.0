...
public interface AppNamespaceRepository extends PagingAndSortingRepository<AppNamespace, Long>{
  ...
  @Modifying
  @Query(value = "UPDATE \"AppNamespace\" SET \"IsDeleted\" = 1, \"DeletedAt\" = ROUND(EXTRACT(EPOCH FROM NOW())*1000), \"DataChange_LastModifiedBy\" = ?2 WHERE \"AppId\" = ?1 and \"IsDeleted\" = 0", nativeQuery = true)
  int batchDeleteByAppId(String appId, String operator);

  @Modifying
  @Query(value = "UPDATE \"AppNamespace\" SET \"IsDeleted\" = 1, \"DeletedAt\" = ROUND(EXTRACT(EPOCH FROM NOW())*1000), \"DataChange_LastModifiedBy\" = ?3 WHERE \"AppId\" = ?1 and \"Name\" = ?2 and \"IsDeleted\" = 0", nativeQuery = true)

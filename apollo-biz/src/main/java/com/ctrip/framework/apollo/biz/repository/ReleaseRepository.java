...
public interface ReleaseRepository extends PagingAndSortingRepository<Release, Long> {
  ...
  @Modifying
  @Query(value = "update \"Release\" set \"IsDeleted\" = 1, \"DeletedAt\" = ROUND(EXTRACT(EPOCH FROM NOW())*1000), \"DataChange_LastModifiedBy\" = ?4 where \"AppId\" = ?1 and \"ClusterName\" = ?2 and \"NamespaceName\" = ?3 and \"IsDeleted\" = 0", nativeQuery = true)

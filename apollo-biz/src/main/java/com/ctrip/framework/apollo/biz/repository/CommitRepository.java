...
public interface CommitRepository extends PagingAndSortingRepository<Commit, Long> {
  ...
  @Modifying
  @Query(value = "update \"Commit\" set \"IsDeleted\" = 1, \"DeletedAt\" = ROUND(EXTRACT(EPOCH FROM NOW())*1000), \"DataChange_LastModifiedBy\" = ?4 where \"AppId\" = ?1 and \"ClusterName\" = ?2 and \"NamespaceName\" = ?3 and \"IsDeleted\" = 0", nativeQuery = true)

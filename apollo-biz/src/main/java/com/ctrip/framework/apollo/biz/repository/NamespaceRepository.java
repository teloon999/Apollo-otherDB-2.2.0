...
public interface NamespaceRepository extends PagingAndSortingRepository<Namespace, Long> {
  ...
  @Modifying
  @Query(value = "update \"Namespace\" set \"IsDeleted\" = 1, \"DeletedAt\" = ROUND(EXTRACT(EPOCH FROM NOW())*1000), \"DataChange_LastModifiedBy\" = ?3 where \"AppId\" = ?1 and \"ClusterName\" = ?2 and \"IsDeleted\" = 0", nativeQuery = true)
